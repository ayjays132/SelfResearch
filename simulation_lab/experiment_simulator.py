...
class ExperimentSimulator:
    ...
    def _ai_debug(self, error_message: str, tb: str, simulation_type: str, kwargs: dict) -> str:
        """
        AI Scaffolding Error System: Uses Qwen to diagnose Python tracebacks.
        """
        log.warning(f"{Colors.YELLOW}Triggering AI Scaffolding Error Handler for deep diagnosis...{Colors.RESET}")
        try:
            llm = LanguageModelWrapper(model_name=DEFAULT_GENERATOR, device=str(self.device))
            prompt = (
                f"You are an expert Python scientific computing debugger.\n"
                f"A simulation of type '{simulation_type}' failed with the following parameters: {kwargs}\n"
                f"Error: {error_message}\n\n"
                f"Traceback:\n{tb}\n\n"
                f"Provide a concise, highly technical diagnosis of the exact cause and an actionable fix.\n"
                f"Diagnosis:"
            )
            diagnosis = llm.generate(prompt, max_new_tokens=150, temperature=0.3).strip()
            return diagnosis
        except Exception as e:
    ...
    def simulate(self, simulation_type: str, **kwargs) -> torch.Tensor:
        ...
        Includes an intelligent AI Scaffolding Error handler to automatically debug tracebacks.
        """
        ...
        try:
            ...
            log.error(f"{Colors.RED}Simulation crashed! Extracting traceback for AI...{Colors.RESET}")
            tb = traceback.format_exc()
            diagnosis = self._ai_debug(str(e), tb, simulation_type, kwargs)
            log.critical(f"{Colors.BG_RED}{Colors.BRIGHT_WHITE}--- AI DIAGNOSIS ---{Colors.RESET}\n{Colors.BRIGHT_YELLOW}{diagnosis}{Colors.RESET}")
            raise e
