...
from models.model_wrapper import ModelRegistry
...
def cluster_dataset_embeddings(
    ...
    *,
    ...
    # Use ModelRegistry for in-memory caching of the model
    model, _ = ModelRegistry.get_model_and_processor(model_name, device=str(device_t))
    
    # Robustly find the base model that returns hidden states
    embed_model = model
    if hasattr(model, "model"): # Common for Llama, Qwen, etc.
        embed_model = model.model
    if hasattr(embed_model, "language_model"): # Common for VLMs
        embed_model = embed_model.language_model
    ...
    for i in range(0, len(dataset), batch_size):
        ...
        with torch.no_grad():
            ...
            if hasattr(outputs, "last_hidden_state"):
                ...
                # Fallback to model output if it's a tensor
                hidden = outputs
...
def compute_tsne_embeddings(
    ...
    *,
    ...
    if hasattr(model, "model"):
        embed_model = model.model
    if hasattr(embed_model, "language_model"):
        embed_model = embed_model.language_model
